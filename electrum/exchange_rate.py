...
class Yadio(ExchangeBase):
    ...
    async def get_rates(self, ccy: str) -> Mapping[str, Optional[Decimal]]:
        json = await self.get_json('api.yadio.io', '/rate/%s/BIT' % ccy)
        return {ccy: to_decimal(json['rate'])}

class Bitnet_IOAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("BIT", ""), to_decimal(json[r]['last']))
                     for r in json if r != 'timestamp'])


class Bitnet_IOtoyou(ExchangeBase):
...
class Bitnet_IOVenezuela(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com', '/')
        rates = [(r, to_decimal(json['BIT'][r])) for r in json['BIT']
...
class BitFinex(ExchangeBase):
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json(
            'api.bitfinex.com',
            f"/v2/candles/trade:1D:tBIT{ccy}/hist?limit=10000")
...
class BitStamp(ExchangeBase):
    ...
    async def request_history(self, ccy):
        ...
        merged_history = {}
        history_starts = 1313625600  # for BITUSD pair (probably earliest)
...
class MercadoBitnet_IO(ExchangeBase):
