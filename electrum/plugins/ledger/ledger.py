...
MSG_NEEDS_FW_UPDATE_SEGWIT = _('Firmware version (or "Bitnet_IO" app) too old for Segwit support. Please update at') + \
    ' https://www.ledger.com'
...
def get_bip44_chain(chain: 'Chain') -> int:
    """
    Determine the BIP 44 coin type based on the Bitnet_IO chain type.

    For the Bitnet_IO mainnet chain, this returns 0. For the other chains, this returns 1.
    ...
    def _perform_hw1_preflight(self):
        ...
        try:
            ...
            if e.sw == 0x6d00 or e.sw == 0x6700:
                raise UserFacingException(_("Device not in Bitnet_IO mode")) from e
            raise e
        else:
            deprecation_warning = (
